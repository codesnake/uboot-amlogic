...
mmc_init_all:
	...
	if(IS_MESON_M8M2_CPU){ //m8m2
		...
		ddr_channel_0 = ((((timing_set->t_mmc_ddr_ctrl>>24)&0x3)==0x1)?(0):(1));
		ddr_channel_1 = ((((timing_set->t_mmc_ddr_ctrl>>24)&0x3)==0x3)?(0):(1));
		//MMC/DMC reset
...
pub_init:
	...
	if(ddr_channel_0_power){
		...
		writel( ((readl(P_DDR1_PUB_DTCR) & 0x00ffffbf) | (1 << 28 ) | (1 << 24) | (1 << 6) |(1<<23) ), P_DDR0_PUB_DTCR);
	...
	if(ddr_channel_1_power){
		...
		writel(timing_set->t_pctl_rstl_us,   P_DDR1_PCTL_TRSTL);       // 0 for ddr2;  2 for simulation; 500 for ddr3.

		//configure the PCTL for DDR1
#if defined(CONFIG_DDR_LOW_POWER_DISABLE)
...
#if ((defined LPDDR2) || (defined LPDDR3))
		writel((readl(P_DDR1_PUB_DSGCR))|(0x3<<06), P_DDR1_PUB_DSGCR); //eanble gate extension  dqs gate can help to bit deskew also
		//writel((((readl(P_DDR1_PUB_ZQCR))&0xff01fffc)|(1<<24)), P_DDR1_PUB_ZQCR);
#else
		...
		//configure auto refresh when data training.
		writel( (readl(P_DDR1_PUB_PGCR2) & 0xfffc0000) | 0xc00 , P_DDR1_PUB_PGCR2 );
		//writel(((readl(P_DDR1_PUB_DTCR) & 0xf0ffffff) | (1 << 24)), P_DDR1_PUB_DTCR);
		/*mpr mode: set DTCR[bit6]=1, DCR[bit8]=1. or set to 0 to disable mpr*/
		//writel(((readl(P_DDR1_PUB_DTCR) & 0xffffffbf) | (1 << 6)), P_DDR1_PUB_DTCR);
		writel( ((readl(P_DDR1_PUB_DTCR) & 0x00ffffbf) | (1 << 28 ) | (1 << 24) | (1 << 6) |(1<<23) ), P_DDR1_PUB_DTCR);

		//for training gate extended gate
		writel(0x8, P_DDR1_PUB_DX0GCR3); //for pdr mode bug //for pdr mode bug this will cause some chip bit deskew  jiaxing add
		...
		writel(readl(P_DDR1_PUB_ACBDLR0) | (timing_set->t_pub_acbdlr0), P_DDR1_PUB_ACBDLR0);  //place before write level

		//initialization PHY.
		writel(timing_set->t_pub_ptr[0]	, P_DDR1_PUB_PTR0);
		writel(timing_set->t_pub_ptr[1]	, P_DDR1_PUB_PTR1);

		//configure DDR1__rst pin.
...
#if ((defined LPDDR2) || (defined LPDDR3))
		writel(timing_set->t_pctl_tckesr,	P_DDR1_PCTL_TCKESR);
		writel(timing_set->t_pctl_tdpd,		P_DDR1_PCTL_TDPD);
#endif
	}
	...
	if(ddr_channel_1_power){ //bus_width setting
		if(cfg_ddr_mode == CFG_DDR_BUS_WIDTH_32BIT){
			writel((0xf0 << 1),P_DDR1_PCTL_PPCFG);
		}
		else if(cfg_ddr_mode == CFG_DDR_BUS_WIDTH_16BIT_LANE01)
		{
			writel((0x1fc | 1), P_DDR1_PCTL_PPCFG);
			writel(0, P_DDR1_PUB_DX2GCR0);
			writel(0, P_DDR1_PUB_DX3GCR0);
			writel(0, P_DDR1_PUB_DX4GCR0);
			writel(0, P_DDR1_PUB_DX5GCR0);
		}
	}
	...
	if(ddr_channel_1_power){
		...
		//DDR 0 DFI
#ifdef LPDDR2
		writel(1,P_DDR1_PCTL_DFITPHYWRLAT);
		writel(4,P_DDR1_PCTL_DFITRDDATAEN); //use 1/ 2 /5 trainging will abort 3 /4 is better
#else
		nTempVal = timing_set->t_pctl_tcwl+timing_set->t_pctl_tal;
		nTempVal = (nTempVal - ((nTempVal%2) ? 3:4))/2;
	...
	if(ddr_channel_1){
			...
			(readl(P_DDR1_PUB_PGSR0) != 0xC000000f))
		{
			...
			{
				pctl_serial_puts("Aml log : DDR1 - PUB_PGSR0_ZCERR with [");
				pctl_serial_put_hex(readl(P_DDR1_PUB_PGSR0),32);
				pctl_serial_puts("] retry...\n");
				DDR_INIT_ERROR_LOOP(loop_err_count);
				goto pub_init;
			}
	...
	if(ddr_channel_1){
			...
			(readl(P_DDR1_PUB_PGSR0) != 0xC000003f))
		{
			...
			{
				pctl_serial_puts("Aml log : DDR1 - PUB_PGSR0_WLERR with [");
				pctl_serial_put_hex(readl(P_DDR1_PUB_PGSR0),32);
				pctl_serial_puts("] retry...\n");
				DDR_INIT_ERROR_LOOP(loop_err_count);
				goto pub_init;
	...
	if(ddr_channel_1){
...
#endif //CONFIG_VLSI_EMULATOR
		{
			...
			{
				pctl_serial_puts("Aml log : DDR1 - PUB_PGSR0_QSGERR with [");
				pctl_serial_put_hex(readl(P_DDR1_PUB_PGSR0),32);
				pctl_serial_puts("] retry...\n");
				DDR_INIT_ERROR_LOOP(loop_err_count);
				goto pub_init;
	...
	if(ddr_channel_1){
			...
			(readl(P_DDR1_PUB_PGSR0) != 0xC00000ff))
		{
			...
			{
				pctl_serial_puts("Aml log : DDR1 - PUB_PGSR0_WLAERR with [");
				pctl_serial_put_hex(readl(P_DDR1_PUB_PGSR0),32);
				pctl_serial_puts("] retry...\n");
				DDR_INIT_ERROR_LOOP(loop_err_count);
				goto pub_init;
	...
	if(ddr_channel_1){
			...
			(readl(P_DDR1_PUB_PGSR0) != 0xC0000fff))
		{
			...
			{
			    pctl_serial_puts("Aml log : DDR1 - PUB_PGSR0_DTERR with [");
				pctl_serial_put_hex(readl(P_DDR1_PUB_PGSR0),32);
				pctl_serial_puts("] retry...\n");
			    goto pub_init;
			}
	...
	if(ddr_channel_1){
		writel(0xf173, P_DDR1_PUB_PIR);
		while((readl(P_DDR1_PUB_PGSR0) != 0x80000f1f) && \
			(readl(P_DDR1_PUB_PGSR0) != 0xC0000f1f) && \
			(readl(P_DDR1_PUB_PGSR0) != 0x80000f5f) && \
			(readl(P_DDR1_PUB_PGSR0) != 0xC0000f5f)){
			//ddr_udelay(10);
			if(readl(P_DDR1_PUB_PGSR0) &( PUB_PGSR0_DTERR|PUB_PGSR0_ZCERR)){
				pctl_serial_puts("Aml log : DDR1 - PUB_PGSR0_ERR with [");
				pctl_serial_put_hex(readl(P_DDR1_PUB_PGSR0),32);
				pctl_serial_puts("] retry...\n");
				goto pub_init;
			}
		}
		hx_serial_puts("Aml log : DDR1 - training done PGSR0 = 0x");
	...
	if(ddr_channel_1){
...
#endif
	    {
			pctl_serial_puts("Aml log : DDR1 - PUB init fail with PGSR0 : 0x");
			pctl_serial_put_hex(nTempVal,32);
			pctl_serial_puts("Aml log : Try again with MMC reset...\n\n");
			DDR_INIT_ERROR_LOOP(loop_err_count);
			goto mmc_init_all;
	    ...
	    {
	        hx_serial_puts("Aml log : DDR1 - init pass with  PGSR0 : 0x");
			hx_serial_put_hex(readl(P_DDR1_PUB_PGSR0),32);
			hx_serial_puts("\n");
			//serial_puts("\n  PGSR1 : 0x");
	...
	if(ddr_channel_1_power){
#if defined(CONFIG_GATEACDDRCLK_DISABLE)
		writel((0x7d<<9)|(readl(P_DDR1_PUB_PGCR3)), P_DDR1_PUB_PGCR3);
#else
	...
	if(0 == ddr_channel_1){ //ddr channel 1 disabled, power down it.
