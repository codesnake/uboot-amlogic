...
static void lcd_ports_ctrl_minilvds(Bool_t status)
{
	...
	pinmux_reg = lcd_Conf->lcd_control.mlvds_config->mlvds_pinmux->tcon_pinmux[0] + PERIPHS_PIN_MUX_0;
	pinmux_bit = lcd_Conf->lcd_control.mlvds_config->mlvds_pinmux->tcon_pinmux[1];
	n = ARRAY_SIZE(lcd_Conf->lcd_control.mlvds_config->mlvds_pinmux->tcon_pinmux_pins);
	
	if (status) {
		WRITE_LCD_CBUS_REG(pinmux_reg, (READ_LCD_CBUS_REG(pinmux_reg) | pinmux_bit));
		aml_lcd_gpio_set(lcd_Conf->lcd_control.mlvds_config->mlvds_pinmux->tcon_gpio[0], lcd_Conf->lcd_control.mlvds_config->mlvds_pinmux->tcon_gpio[1]);
	}else {
		WRITE_LCD_CBUS_REG(pinmux_reg, (READ_LCD_CBUS_REG(pinmux_reg) & ~(pinmux_bit)));
		for (i=0; i<n; i++) {
			aml_lcd_gpio_set(lcd_Conf->lcd_control.mlvds_config->mlvds_pinmux->tcon_pinmux_pins[i], LCD_POWER_GPIO_INPUT);
		}
		aml_lcd_gpio_set(lcd_Conf->lcd_control.mlvds_config->mlvds_pinmux->tcon_gpio[0], LCD_POWER_GPIO_INPUT);
	}
	lcd_print("%s: %s\n", __FUNCTION__, (status ? "ON" : "OFF"));
}
...
static void vclk_set_lcd(int lcd_type, unsigned long pll_reg, unsigned long vid_div_reg, unsigned int clk_ctrl_reg)
{
	...
	if(lcd_type == LCD_DIGITAL_TTL)
		...
		WRITE_LCD_CBUS_REG_BITS(HHI_VIID_CLK_DIV, 8, 12, 4); // [23:20] encl_clk_sel
...
static void set_pll_mlvds(Lcd_Config_t *pConf)
{
	...
	//enable v2_clk div
    // WRITE_LCD_CBUS_REG( HHI_VIID_CLK_CNTL, READ_MPEG_REG(HHI_VIID_CLK_CNTL) | (0xF << 0) );
    // WRITE_LCD_CBUS_REG( HHI_VID_CLK_CNTL, READ_MPEG_REG(HHI_VID_CLK_CNTL) | (0xF << 0) );
...
static void set_control_mlvds(Lcd_Config_t *pConf)
{
    ...
    WRITE_LCD_REG(LVDS_PHY_CNTL1, READ_LCD_REG(LVDS_PHY_CNTL1) | (0x7F << 8) );
...
static void init_phy_lvds(Lcd_Config_t *pConf)
{
    ...
    WRITE_LCD_REG(LVDS_PHY_CNTL3, 0xee1);  //0xee0
...
static MLVDS_Pinmux_t lcd_mlvds_tcon_pinmux = {
...
static MLVDS_Config_t lcd_mlvds_config = {
    ...
    .phase_select = 1,
    .TL080_phase = 3,
    .mlvds_pinmux = &lcd_mlvds_tcon_pinmux,
};
